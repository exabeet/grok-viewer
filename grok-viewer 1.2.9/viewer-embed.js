  ...
  const downloadAll = () => {
    ...
    const run = async () => {
      try {
        ...
        const batchSize = total > 45 ? 25 : total;
        const batches = [];
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
          ...
          const fetchOne = async () => {
            while (queue.length) {
              ...
              completed += 1;
              setStatus(
                `${isImages ? "Preparing image archive" : "Preparing archive"} ${batchIndex + 1}/${batches.length}... ${completed}/${batch.length}`
              );
            }
          };
          const workers = [];
          ...
          setStatus(`${isImages ? "Building image archive" : "Building archive"} ${batchIndex + 1}/${batches.length}...`);
