(() => {
  ...
  debug("script loaded", { pathname: location.pathname, search: location.search });
  if (!location.pathname.includes("/imagine/saved")) {
    debugWarn("skip: pathname mismatch", { pathname: location.pathname });
    return;
  }
  ...
  const fetchPage = async (cursor) => {
    ...
    if (!response.ok) {
      debugWarn("fetch page failed", { status: response.status, cursor: cursor || "" });
  ...
  const fetchAllFavorites = async () => {
    ...
    try {
      ...
      if (succeeded) {
        debug("fetchAllFavorites success", { count: allItems.length });
        persistVideos(allItems);
      }
    } catch (error) {
      debugError("fetchAllFavorites error", {
  ...
  const fullSyncFavorites = async () => {
    ...
    try {
      ...
      debugError("fullSyncFavorites error", {
        message: String((error && error.message) || error || "unknown")
      });
    } finally {
      fetchInFlight = false;
  ...
  const createFloatingButton = () => {
    ...
    img.src = chrome.runtime.getURL("images/logo-extension.svg");
    img.alt = "grok-viewer";
    img.style.cssText = `
      width: 100%;
      height: 100%;
  ...
  const init = async () => {
    debug("init start", { readyState: document.readyState });
