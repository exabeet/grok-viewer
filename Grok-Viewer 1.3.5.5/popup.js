(() => {
  ...
  const FAVORITES_TAB_MATCH = "https://grok.com/*imagine/saved*";
  ...
  const sendToFavorites = (message, callback) => {
    chrome.tabs.query({ url: FAVORITES_TAB_MATCH }, (tabs) => {
  ...
  const deleteItem = (item) => {
    ...
    sendToFavorites({ action: "grokViewerDeleteOne", postId: item.postId }, (result) => {
      ...
      if (!result.ok || !result.response || !result.response.ok) {
        setStatus("Delete failed. Keep saved open.");
  ...
  const deleteAll = () => {
    ...
    if (!window.confirm("Delete all videos from Grok saved?")) return;
    state.busy = true;
    updateActionButtons();
    ...
    sendToFavorites({ action: "grokViewerDeleteAll", postIds }, (result) => {
      ...
      updateActionButtons();
      if (!result.ok || !result.response || !result.response.ok) {
        setStatus("Delete all failed. Keep saved open.");
        showToast("Delete failed.", "error");
  ...
  const updateVideos = (items, updatedAt = Date.now()) => {
    ...
    if (!state.videos.length) {
      setStatus("No videos yet. Keep saved open.");
    } else {
  ...
  const detectFavoritesTab = () => {
    chrome.tabs.query({ url: FAVORITES_TAB_MATCH }, (tabs) => {
      ...
      state.favoritesTabId = tab ? tab.id : null;
      if (state.favoritesTabId) {
        setStatus("Saved open. Live syncing...");
      } else if (!state.videos.length) {
        setStatus("Open Grok saved to sync videos.");
      }
      updateActionButtons();
    });
  };
  ...
  const refreshFromFavorites = () => {
    ...
    sendToFavorites({ action: "grokViewerRefresh" }, (result) => {
      if (!result.ok) {
        setStatus("Refresh failed. Keep saved open.");
