(() => {
  ...
  const openViewerWindow = (incognito) => {
    chrome.windows.create({
      url: "https://grok.com/imagine/saved?grokViewer=1#grokViewer=1",
      type: "popup",
      width: 1200,
      height: 820,
          ...
          func: async (tokenArg) => {
            ...
            ensureProbe();
              ...
              for (let wait = 0; wait < 120; wait += 1) {
                if (bodyLooksBlocked()) {
                  return { ok: false, error: "cloudflare-challenge-page", diag: { wait, href: String(location.href || "") } };
                }
              ...
              if (!repeatCandidates.length) {
                return { ok: false, error: "start-button-not-found", diag: { href: String(location.href || ""), candidateCount: 0 } };
              }
              const candidateSnapshot = repeatCandidates.slice(0, 5).map((entry) => normalize((entry && entry.text) || "")).filter(Boolean);
              ...
              for (let i = 0; i < maxCandidates; i += 1) {
                ...
                if (confirmClicked) {
                  return {
                    ok: false,
                    error: "repeat-clicked-but-not-triggered",
                    diag: {
                      href: String(location.href || ""),
                      candidateCount: repeatCandidates.length,
                      candidateSnapshot,
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message && message.action === "grokViewerRegenViaNativeTab") {
      ...
      console.log("[GV-REGEN][BG] start native tab regen", { requestId, postId, viewerTabId });
      ...
      createNativeRegenTab(postUrl)
        .then(async (created) => {
          ...
          try {
            ...
            console.log("[GV-REGEN][BG] click result", {
              requestId,
              targetTabId,
              ok: clickResult && clickResult.ok === true,
              error: clickResult && clickResult.error ? String(clickResult.error) : "",
              diag: clickResult && clickResult.diag ? clickResult.diag : null
            });
