(() => {
  ...
  const extractResolutionPair = (post) => {
    ...
    const width = toPositiveSize(fromResolution.width || post.width || post.mediaWidth || 0);
    const height = toPositiveSize(fromResolution.height || post.height || post.mediaHeight || 0);
    return { width, height };
  };
  ...
  const prewarmAroundCurrentSelection = () => {
    ...
    for (let i = 0; i < indexes.length; i += 1) {
      ...
      const source = getPlaybackCandidates(active)[0] || "";
      if (!source || seen.has(source)) continue;
      seen.add(source);
      prewarmPlaybackSource(i, source);
    }
  };
  ...
  const deleteAll = async () => {
    if (state.mode === "images") {
      ...
      while (cycle < 6) {
        if (isProgressCancelRequested("delete-images")) {
          canceledByUser = true;
          break;
        }
        ...
        for (let i = 0; i < childIds.length; i += 1) {
          ...
          const res = await likePost(childIds[i]);
          if (!res.ok) {
            // If videos bulk-delete is running in parallel, child videos can already be gone.
            if (!isDeleteAllRunning("videos")) {
        ...
        for (let i = 0; i < toDelete.length; i += 1) {
          ...
          if (!res.ok) {
            ...
            animateThumbRemoval(toDelete[i]);
          }
          deletedCount += 1;
          const totalCount = Math.max(1, totalIds.size);
  ...
  const buildDownloadCandidates = (item) => {
    const urls = [pickDownloadUrl(item), item && item.url ? item.url : ""].filter(Boolean);
  ...
  const downloadPromptInfoFile = async (item) => {
    ...
    if (saveAs && started.downloadId) {
      ...
      rememberAskEachFolderFromDownloadOutcome(outcome, effectiveName);
    }
  ...
  const downloadFile = async (item) => {
    ...
    try {
      ...
      let started = null;
      for (let i = 0; i < candidates.length; i += 1) {
        started = await downloadViaExtension(candidates[i], targetFilename, saveAs);
        if (started && started.ok) break;
        if (isDownloadCanceled(started)) {
          maybePromptDownloadSetupGuide();
          setReadyStatus();
          return;
      ...
      if (saveAs && started.downloadId) {
        ...
        rememberAskEachFolderFromDownloadOutcome(outcome, targetFilename);
      }
      recordDownloadedItems(state.mode, [targetItem]);
      renderGrid();
      showDownloadReady("Your file is ready. Click here", targetFilename);
      ...
      try {
        ...
        if (!started || !started.ok) {
          ...
          if (!retryRequested) {
            ...
            if (isDownloadCanceled(started)) maybePromptDownloadSetupGuide();
          }
          return;
        }
        if (savePrompt && started.downloadId) {
          ...
          rememberAskEachFolderFromDownloadOutcome(outcome, started.filename || archiveName);
        }
  ...
  const getBestPosterUrl = (item, options = {}) => {
    ...
    const ordered = preferSource
      ? [sourceImage, parentFallback, posterImage]
      : [posterImage, sourceImage, parentFallback];
  ...
  const copyPromptItem = async (item, promptOverride) => {
    if (promptCopyAudio) {
      try {
  ...
  const handlePromptItem = async (item) => {
    const prompt = getPromptTextForItem(item);
    if (!prompt) {
      showToast("Prompt unavailable", "error");
      return;
    ...
    if (choice === "copy") {
      await copyPromptItem(item, prompt);
    }
  };
  ...
  const loadPlayer = () => {
    ...
    if (isImages) {
      ...
      const scheduleSourceProbe = () => {
        ...
        if (sourceProbeAttempts >= 14) return;
        clearSourceProbe();
      ...
      const scheduleProgressWatch = () => {
        ...
        progressWatchTimer = setInterval(() => {
          ...
          if (stallNoProgressTicks < 4) return;
          stallNoProgressTicks = 0;
          const currentCandidate = playbackCandidates[Math.max(0, candidateIndex)] || playbackCandidates[0] || "";
          if (currentCandidate) {
            applySource(currentCandidate);
    ...
    if (gridEl) {
      gridEl.addEventListener("click", (event) => {
        ...
        if (actionBtn && gridEl.contains(actionBtn)) {
          ...
          if (action === "prompt") handlePromptItem(item);
          return;
        }
