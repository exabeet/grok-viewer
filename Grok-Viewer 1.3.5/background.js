      ...
      chrome.scripting.executeScript(
        {
          ...
          func: async (tokenArg) => {
            ...
            const ensureProbe = () => {
              ...
              if (!current || typeof current !== "object" || String(current.token || "").trim() !== tokenText) {
            ...
            try {
              ...
              for (let i = 0; i < repeatCandidates.length; i += 1) {
                ...
                for (let t = 0; t < 28; t += 1) {
                  ...
                  if (confirm) {
                    const confirmLabel = normalize(confirm.getAttribute("aria-label") || confirm.getAttribute("title") || confirm.textContent);
                    clickNode(confirm);
                    for (let k = 0; k < 20; k += 1) {
                      if (conversationRequestSeen) {
                        return {
                          ok: true,
                          label: label || "repeat",
                          confirmed: true,
                          confirmLabel: confirmLabel || "create video"
                        };
                      }
                      await sleep(120);
                    }
                  ...
                  if (conversationRequestSeen) {
                    return { ok: true, label: label || "repeat", confirmed: false, confirmLabel: "" };
                  }
                  if (!beforeDisabled && isDisabled(node)) {
                    return { ok: true, label: label || "repeat", confirmed: false, confirmLabel: "" };
                  }
                  if (bodyLooksGenerating()) {
                    return { ok: true, label: label || "repeat", confirmed: false, confirmLabel: "" };
                  }
                  await sleep(140);
                }
