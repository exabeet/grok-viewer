(() => {
  ...
  const buildImageItem = (post) => {
    ...
    const hasVideoChild = Boolean(
      (post.childPosts || []).some((child) => {
        if (!child) return false;
        const childUrl = child.hdMediaUrl || child.mediaUrl || "";
        if (isMp4(childUrl, child.mimeType)) return true;
        return child.mediaType === "MEDIA_POST_TYPE_VIDEO";
      }) ||
        (post.videos || []).some((video) => {
          if (!video) return false;
          const videoUrl = video.hdMediaUrl || video.mediaUrl || "";
          if (isMp4(videoUrl, video.mimeType)) return true;
          return video.mediaType === "MEDIA_POST_TYPE_VIDEO";
        })
    );
    return {
      id: post.id || url,
  ...
  const getVisibleImageIds = () => {
    const ids = new Set();
    const imgs = Array.from(document.querySelectorAll('img[src*="imagine-public.x.ai/imagine-public/images/"]'));
    imgs.forEach((img) => {
      if (img.closest("#gv-overlay")) return;
      const src = img.getAttribute("src") || "";
      const match = src.match(/imagine-public\/images\/([0-9a-f-]{36})\.(?:jpg|jpeg|png|webp)/i);
      if (match) ids.add(match[1]);
    });
    return ids;
  };

  const getImageItemsForDisplay = () => {
    const ids = getVisibleImageIds();
    if (!ids.size) return state.imageItems;
    return state.imageItems.filter((item) => item && item.postId && ids.has(item.postId));
  };

  const updateItems = (videos, images) => {
    state.videoItems = dedupeItems(videos || []);
    state.imageItems = dedupeImageItems(images || []);
    state.items = state.mode === "images" ? getImageItemsForDisplay() : state.videoItems;
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (state.mode === "images") {
        setStatus(`Loaded ${state.imageItems.length} image${state.imageItems.length === 1 ? "" : "s"}.`);
      } else {
        setStatus(`Loaded ${state.videoItems.length} MP4 video${state.videoItems.length === 1 ? "" : "s"}.`);
      }
    } catch (error) {
  ...
  const deleteAll = async () => {
    if (state.mode === "images") {
      ...
      const toDelete = getImageItemsForDisplay()
        .filter((item) => item && item.url && isImage(item.url, item.mimeType) && !item.hasVideoChild)
        .map((item) => item.postId)
        .filter(Boolean);
      if (!toDelete.length) {
        state.busy = false;
        setStatus("No images to delete.");
        showToast("No images to delete.");
        updateActionButtons();
        return;
      }
      const failed = [];
      for (let i = 0; i < toDelete.length; i += 1) {
        const result = await unlikePost(toDelete[i]);
        if (!result.ok) failed.push(toDelete[i]);
        await sleep(180);
      }
      const remaining = state.imageItems.filter((item) => failed.includes(item.postId));
      updateItems(state.videoItems, remaining);
      state.busy = false;
  ...
  const applyModeUI = () => {
    ...
    if (downloadAllBtn) downloadAllBtn.style.display = isImages ? "none" : "";
    if (autoNextBtn) autoNextBtn.style.display = isImages ? "none" : "";
    if (deleteAllBtn) deleteAllBtn.textContent = isImages ? "Delete images" : "Delete all";
  };
  ...
  const setMode = (mode) => {
    ...
    state.mode = mode;
    state.items = mode === "images" ? getImageItemsForDisplay() : state.videoItems;
  ...
  const updateActionButtons = () => {
    ...
    const isImages = state.mode === "images";
    if (downloadBtn) downloadBtn.disabled = isImages || !selected || state.busy;
    if (deleteBtn) deleteBtn.disabled = isImages || !canDelete || state.busy;
    if (downloadAllBtn) downloadAllBtn.disabled = isImages || !state.videoItems.length || state.busy;
  ...
  const renderGrid = () => {
    if (!gridEl) return;
    if (state.mode === "images") {
      state.items = getImageItemsForDisplay();
    }
    gridEl.innerHTML = "";
    ...
    if (purgeBtn) {
      purgeBtn.onclick = () => {
        ...
        chrome.storage.local.remove(STORAGE_KEY, () => {
          ...
          closeLightbox();
          setStatus("Cache purged. Reloading...");
          setTimeout(() => {
