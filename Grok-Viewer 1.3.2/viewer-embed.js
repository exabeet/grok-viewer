(() => {
  ...
  const deleteItem = async (item) => {
    ...
    modeState.pageCache.forEach((pageItems, key) => {
      const filtered = (pageItems || []).filter((entry) => entry.postId !== item.postId);
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
          ...
          const fetchOne = async () => {
            while (queue.length) {
              ...
              const prepText = `${isImages ? "Preparing image archive" : "Preparing archive"}${pageLabel} ${batchIndex + 1}/${batches.length}... ${completed}/${batch.length}`;
              setStatus(prepText);
          ...
          const pageLabel = pageCount > 1 ? ` page ${page + 1}/${pageCount}` : "";
          const buildText = `${isImages ? "Building image archive" : "Building archive"}${pageLabel} ${batchIndex + 1}/${batches.length}...`;
          setStatus(buildText);
  ...
  const shareItem = async (item) => {
    ...
    const link = buildShareLink(item.postId);
    if (!link) return;
  ...
  const applyModeUI = () => {
    ...
    if (sortBtn) {
      ...
      sortBtn.textContent =
        state.sortOrder === "asc" ? "Sort by oldest creation" : "Sort by newest creation";
    }
  };
  ...
  const updateActionButtons = () => {
    ...
    if (shareBtn) shareBtn.disabled = !selected || !selected.postId || state.busy;
  ...
  const renderGrid = () => {
    ...
    const renderChunk = () => {
      ...
      for (; index < sliceEnd; index += 1) {
        ...
        shareAction.title = item.postId ? "Share" : "Share unavailable";
        ...
        if (!item.postId || state.busy) shareAction.disabled = true;
